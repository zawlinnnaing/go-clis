...
func TestNewIntervalConfig(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			config := pomodoro.NewIntervalConfig(repo, testCase.input[0], testCase.input[1], testCase.input[2], 0, false)
...
func TestGetInterval(t *testing.T) {
	...
	config := pomodoro.NewIntervalConfig(repo, pomodoroDuration, shortBreakDuration, longBreakDuration, 0, false)
	for i := 1; i <= 16; i++ {
		var (
			expCategory string
...
func TestPause(t *testing.T) {
	...
	config := pomodoro.NewIntervalConfig(repo, duration, duration, duration, 0, false)
	testCases := []struct {
		name        string
		start       bool
		expState    int
